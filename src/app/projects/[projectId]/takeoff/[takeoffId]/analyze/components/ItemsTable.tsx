...
const ItemsTable = ({ takeOff, selectedFileId, onRefreshItems }: any) => {
  ...
  const renderField = useCallback(
    (_: any, record: LabelItem, fieldName: string) => {
      try {
        let fieldValue = "";
        try {
        ...
        return (
          ...
          >
            {isEditing ? (
              ...
              <div
                className="min-h-[20px] flex-1 flex justify-center items-center"
                onClick={() =>
      ...
      render: (_: any, record: any, index: number) => {
        return (
          <div className="flex items-center justify-center">
            <div
              className="w-[16px] h-[16px] rounded-full bg-transparent border border-basicLightGray text-[10px] text-white cursor-pointer"
              style={{
                backgroundColor: record.is_checked
    ...
    if (dynamicFields && Array.isArray(dynamicFields)) {
      dynamicFields.forEach((field: { name: string }, index: number) => {
        ...
        columnsList.push({
          title: (
            <div
              className="text-center whitespace-pre-wrap text-xs text-grey-normal"
              style={{
