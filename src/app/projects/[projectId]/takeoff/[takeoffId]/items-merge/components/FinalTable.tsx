...
const FinalTable = ({
  ...
  const getColumns = () => {
    ...
    let endColumns = [
      {
        title: <span className="text-xs text-grey-normal">Reference</span>,
        dataIndex: "",
        align: "center",
        render: (text: any, record: any, index: number) => {
          return (
    ...
    let header = headerList.map((value: string) => {
      return {
        title: <span className="text-xs text-grey-normal">{value}</span>,
        dataIndex: value,
        align: "center",
  ...
  return (
    <div
      className={`px-7 py-4 mb-5 rounded-lg mt-auto`}
      style={{
        backgroundColor: colorList.forumBlue - normal + "10",
      }}
    >
      <div className="h-[24px] flex flex-row justify-between">
        <p className="pb-4 text-sm text-forumBlue-normal">Final Item</p>
        <div
          className="px-3 py-1 h-['auto'] cursor-pointer text-white text-xs rounded-lg bg-forumBlue-normal"
