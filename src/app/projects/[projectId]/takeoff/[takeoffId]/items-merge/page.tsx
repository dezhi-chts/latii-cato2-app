...
import { Divider, Input, notification, UploadFile, Table, Button, Image, Spin, Modal } from "antd";
...
const ItemsMerge = ()=>{
    ...
    const [selectedTab, setSelectedTab] = useState<'customize' | 'keepAll'>('customize');
    ...
    const originTabelData = useMemo(()=>{
        ...
        let list  = allItems.current.filter((item:any)=> group?.item_ids?.includes(item.id));
    ...
    const showEvidence = ()=>{
        ...
        originTabelData.forEach((item:any, index:number)=>{
            let evid: any = fileAllEvidence.current.find((evidence:any)=> item?.evidence_id_list?.includes(evidence.id));
            if(evid){
                evidences.push({...evid, index: index, item_id: item.id, evidence_selected: true});
            }
        });
    ...
    const renderGrouplList = ()=>{
        let list = groupList.map((group:any, index:number)=>{
            return <div 
                        ...
                        style={{
                            borderColor: selectGroupIndex === index ? colorList.forumBlue : colorList.primaryN30,
                        }}
                        onClick={()=>setSelectGroupIndex(index)}
                    >
                        <div className="">Label</div>
                        <p className="text-basicGray">{'Items ' + group?.items?.map((item:any)=>item.id).join(", ")}</p>
                    </div>
        });
        return list;
    }
    ...
    return <div className="w-full h-[100vh] px-20 pb-5 flex flex-col">
        <div className="py-10 mb-3 flex flex-row items-center gap-6">
            ...
            <span className="text-sm text-basicLightGray">
                Back to Items
                </span>
            </div>
            ...
            <div className="flex-1 flex flex-row">
                <div className="w-[195px] h-full pr-[25px] border-r border-primaryN30">
                    <div className="mb-6">
                        <p className="text-base text-forumBlue mb-1">
                            Reconcile Panel
                        </p>
                ...
                <div className="px-5 flex-1 flex flex-col gap-3">
                    <div className='flex text-xs text-basicGray'>
                        <div 
                    ...
                    <div className="my-4 flex flex-row gap-5 flex-wrap">
                        {
                            evidenceList.map((item:any, index:number)=>{
                                return <div key={index} className="w-[30%] min-w-[30%] h-[300px] border border-primaryN30 rounded-lg">
                                    <div className="flex flex-row p-4 gap-6">
                                        <div className="px-1 h-[16px] text-xs text-basicGray bg-primaryN30 rounded">{item.index + 1}</div>
                                        <div className="flex-1">
                                            <Image src={item.evidence_url ?? ''} alt={""} width={'100%'} height={'auto'} className="mt-2 max-h-[250px] object-contain" preview={false} />
                                        </div>
                                        <div className="flex flex-col">
                                            <div
                                                ...
                                                `}
                                                style={{
                                                    border: item.evidence_selected ? `1px solid ${colorList.forumBlue}` : '1px solid transparent',
                                                    backgroundColor: item.evidence_selected ? (colorList.forumBlue+'20') : 'transparent',
                                                }} 
