...
const SortableItem = (({ field, onChange, checked }: any) => {
  ...
  return (
    <div className={`px-2 w-full h-[30px] flex flex-row items-center rounded-md ${disable ? 'bg-primaryN20' : ''}`}
    >
      <div className="pr-4"><Image src="/assets/icons/drag.svg" alt="Drag" width={6} height={10}></Image></div>
...
export const ColumnView = ({ open, onClose, columns, onColumnsChange, selectedColumns }: ColumnViewProps) => {
  const [checkedColumns, setCheckedColumns] = useState<string[]>(selectedColumns);
  ...
  return (
    <Modal
      title={
        <p className="text-forumBlue text-lg font-semibold">Column View</p>
      }
      onCancel={onClose}
      open={open}
      closable={false}
      footer={
        <div className="mt-4 flex gap-3 justify-end">
          <Button className="w-[76px] h-[28px]" onClick={onClose}>Cancel</Button>
          <Button className="w-[76px] h-[28px]" type="primary" onClick={handleSave}>Save</Button>
        </div>
      }
    >
      {
