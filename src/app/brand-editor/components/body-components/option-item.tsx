...
import { Input, Flex, Popconfirm, Select, Modal, Radio, Divider, Descriptions, TreeSelect, Tooltip, Button, Checkbox, Image as AntdImage, notification } from "antd";
import { DatabaseOutlined, PlusCircleTwoTone, PlusCircleFilled, RightOutlined, SaveOutlined, ProfileOutlined, FolderViewOutlined, RadiusBottomleftOutlined, PlusOutlined, CopyOutlined, DeleteOutlined, LeftOutlined, PlusCircleOutlined, SearchOutlined, EyeOutlined } from '@ant-design/icons';
import {
	OptionMsgVO
...
const OptionItemCom = (props: any) => {
	...
	const [isSubOptionsModalOpen, setIsSubOptionsModalOpen] = useState<boolean>(false);
			...
			<div className="flex flex-col items-center">
				...
				>
					<div className="flex w-full items-center border-b border-[#E8E8E8] p-4">
						<Select
							...
							options={(setedOptionLibraryList || []).map((v: Record<string, any>) => ({
								value: v.code,
								label: `${v?.title}`,
							}))}
								...
								optionItemMsg.children.length > 0 && <>
									{
										!optionItemMsg._collapsed ? <Tooltip title="Fold" placement="top">
											<LeftOutlined className="ml-2 cursor-pointer" onClick={() => props.onToggleHandler(optionItemMsg.id)} />
										</Tooltip> : <Tooltip title="Unfold" placement="top">
											<RightOutlined className="ml-2 cursor-pointer" onClick={() => props.onToggleHandler(optionItemMsg.id)} />
										</Tooltip>
									}
								</>
							}
					...
					<div className="flex mt-2 w-full items-start p-4 flex-col">
						...
						<div className="mt-2">
							{
								Array.isArray(optionItemMsg.option) && <div>
									{
										Array.isArray(optionItemMsg.optionMsg) && optionItemMsg.optionMsg.map((item: any, index: any) => {
											return <div key={index} className="flex mb-2 items-center">
												<span className="px-2 text-[12px] bg-[#F8F8F8] text-[#717171] rounded-[4px] flex items-center justify-center">{index + 1}</span>
							...
							{
								!Array.isArray(optionItemMsg.option) && <div>
									{optionItemMsg.option && !Array.isArray(optionItemMsg.optionMsg) && (
										<div className="flex mb-2 items-center">
											<span className="px-2 text-[12px] bg-[#F8F8F8] text-[#717171] rounded-[4px] flex items-center justify-center">{1}</span>
											<span className="ml-2">{optionItemMsg?.optionMsg?.name}</span>
										</div>
									)}
								</div>
							}
				...
				</div>
				{
					...
					>
						...
						{
							!optionItemMsg._isLastOneAtThisLevel && <div className="border-l flex-1 min-h-5 border-[#DCDCDC]"></div>
						}
					</div>
				}
			</div>

			<div className="text-basicGray relative flex h-[150px]">
				<div
				...
				>
					...
					<div
						...
						className={
							`w-[25px] h-[25px] rounded-[100%] bg-[#E3EBF8] flex items-center justify-center cursor-pointer`
						}
					>
						<PlusOutlined className="text-[#427CCE]" />
					</div>
					{
						(optionItemMsg.children.length > 0 && !optionItemMsg._collapsed) && <div className="border-b w-[20px] border-[#DCDCDC]"></div>
					}
				</div>
			</div>
			<Modal
			...
			>
				<div className="h-[calc(100vh-200px)]">
					<div className="flex items-center">
						...
						/>
						<Button className="ml-6" onClick={onUnselectAll}>Unselect All</Button>
						<Button className="ml-6" onClick={onSelectAll}>Select All</Button>
					</div>
					<div className="h-[calc(100%-90px)] overflow-y-auto">
						<div className="mt-4 flex flex-wrap items-start text-[12px] gap-4">
							{
								(allSubOptions && allSubOptions.length != 0) && allSubOptions.map((item, index) => {
									return (
										item.isShow
											? <div key={index} className="w-[370px] h-[180px] border border-[#E8E8E8] rounded-md flex-none">
												<div className="flex justify-between items-center p-2 px-4 border-b border-b-[#E8E8E8]">
													<div className="flex items-center">
														<span className="px-2 text-[12px] bg-[#F8F8F8] text-[#717171] rounded-[4px] flex items-center justify-center">{index + 1}</span>
														<span className="ml-4">{item?.name}</span>
													</div>
													<Checkbox checked={item?.checked} onChange={(e) => onChecked(e, index)}></Checkbox>
												</div>
